...
class _AKButton(M_MDIconButton):
...
class M_AKBottomNavigation(ThemableBehavior, BoxLayout):
    items = ListProperty()
    ...
    def _update_items(self, items):
        ...
        for x in range(0, count):
            ...
            label.font_name = self.items[x]["font_name"] if "font_name" in self.items[x] else label.font_name
            label.font_size = self.items[x]["font_size"] if "font_size" in self.items[x] else label.font_size
    ...
    def set_current(self, index):
        ...
        M_AKBottomNavigation._selected = index
